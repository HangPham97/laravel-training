...
class TestModuleServiceProvider extends ServiceProvider
{
    ...
    {
        $viewPath = resource_path('views/modules/testmodule');
        ...
        $this->loadViewsFrom(array_merge(array_map(function ($path) {
            return $path . '/modules/testmodule';
        }, \Config::get('view.paths')), [$sourcePath]), 'testmodule');
    }
    ...
    {
        $langPath = resource_path('lang/modules/testmodule');

        if (is_dir($langPath)) {
            $this->loadTranslationsFrom($langPath, 'testmodule');
        } else {
